...
function countElectronicsProducts(products) {
  const filterElectronic = products.filter((x) => {
  ...
  return filterElectronic.length;
}
...
function listProductNamesByCategory(products, category) {
  const filterElectronic1 = products.filter((x) => {
    if (x.category == "Electronics") {
      return true;
    } else {
      return false;
  ...
  filterElectronic1.forEach((element) => console.log(element.name));
}

console.log("spicy part start here :) ");
...
function getCheapestProduct(products) {
  const found = products.find((element) => {
    ...
    const arrPrice = element.price;
    let min = 0;

    for (let index = 0; index < arrPrice.length; index++) {
      if (arrPrice[index] > min) {
        min = arrPrice[index];
      }
    }
    return min;
...
function removeProductById(products, productId) {
  const removeElement = products.filter(myFunction);
